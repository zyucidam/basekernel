...
int sys_process_run( int fd, int argc, const char **argv)
{
	...
	/*change the priority*/
	if(argc >= 3 && !strcmp(argv[1],"priority"))
	{
		int pri;
		if(str2int(argv[2], &pri))
		{
			p->node.priority = pri;
	...
	/* priority process or non-priority process judgment*/

	if(argc >= 3 && !strcmp(argv[1],"priority"))
	{
		process_launch_priority(p);
	} else {
		process_launch(p);
	}
	return p->pid;
...
/*open a named pipe
kobject is needed, it being used as write/read
***this does not mean pipe connect by fd, in different process they may have different fd***
*/
...
/*make a new named pipe 
kobject Kobject in make named pipe is actually not needed, named pipe be opened by fname but not fd
however, in order to follow the sytle of syscall, I still write it
*/
