...
/*launch a process by priority scheduling*/
void process_launch_priority(struct process *p)
{	
	printf("Process with pid %u priority %u be scheduled\n", p->pid, p->node.priority);
...
static void process_switch(int newstate)
{
	...
	if(current) {
		...
		if(newstate == PROCESS_STATE_READY) {
			/*this let all process become o(n). 
			However, I cannot determine whether current process is priority or not
			In non-priority scheduled process, current->node.priority = 0, however, prioriy can also be 0*/
